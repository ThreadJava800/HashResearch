...
Отсечь большое количество проверок можно зная длину строки (если длины двух строк не равны, то они заведомо не совпадают). Таким образом, начнём с оптимизации поиска длины строки.\
...
Далее была предпринята попытка переписать эту функцию на интринсиках. Тем не менее, в процессе было выяснено, что реализация стандартной функции strlen использует avx оптимизации. После необходимых измерений получились следующие результаты:
|  Оптимизация | Время работы $10^6$ вызовов (мс) | Относительное ускорение | Абсолютное ускорение |
|---|---|---|---|
| - | 145.6 ± 7.2 | baseline | baseline |
| ASM (lodsb) | 409.2 ± 0.4 | 0.36x | 0.36x |
...
| Функция сравнения | 572.8 ± 2.5 | 4x | 4x |
| Оптимизация хеша | 507.0 ± 9.1 | 1.13x | 4.5x |
| Оптимизация длины строки | 502.2 ± 3.9 | 1.01x | 4.54x |
...
|  Оптимизация | Время работы $10^6$ сравнений (мс) | Ускорение |
|---|---|---|
| `-O1` | 278.6 ± 1.9 | baseline |
| ASM версия | 280.8 ± 1.6 | 1x |
...
| `-O1` | 278.6 ± 1.9 | baseline |
| ASM версия | 280.8 ± 1.6 | 1x |
| Intrinsic версия | 216.4 ± 1.9 | 1.3x |

По итогу общее время поиска ускорилось почти в 4 раза.
|  Оптимизация | Время работы $10^5$ поисков (мс) | Относительное ускорение | Абсолютное ускорение |
|---|---|---|---|
| - | 2277.6 ± 12.1 | baseline | baseline | 
| Функция сравнения | 572.8 ± 2.5 | 4x | 4x |
...
|  Оптимизация | Время работы $10^6$ вызовов (мс) | Относительное ускорение | Абсолютное ускорение |
|---|---|---|---|
| `-O1` | 279.4 ± 0.5 | baseline | baseline | 
| ASM версия | 266.0 ± 0.9 | 1.05x | 1.05x |
...
|---|---|---|---|
| `-O1` | 279.4 ± 0.5 | baseline | baseline | 
| ASM версия (inline) | 266.0 ± 0.9 | 1.05x | 1.05x |
| ASM версия (файл) | 287.6 ± 4.8 | 0.92x | 0.97x |
...
|  Оптимизация | Время работы $10^6$ вызовов (мс) | Ускорение |
|---|---|---|
| `-O1` | 143.0 ± 1.1 | baseline |
| ASM версия | 143.4 ± 1.4 | 1x |
| Стандартная функция | 4.4 ± 0.8 | 32.6x |
