...
Далее было принято решение переписать данную функцию при помощи интринсиков. Таким образом, я смог сравнивать 32 символа за раз (AVX-инструкции). Т.к. мы храним длину ключа, у нас нет необходимости отдельно проверять кончилась ли строка, или нет.

Таким образом сравниваются символы (str1 и str2 - `__m256i` переменные с 32 символами):
```C
