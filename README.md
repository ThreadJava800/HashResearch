...
|  Оптимизация | Время работы $10^5$ поисков |
|---|---|
| - | 2155.8 ± 63.5 ms |
...
Для начала я решил посмотреть, какой ассемблерный код соответствует функции сравнения. `-O1` хорошо оптимизирует код, однако всё равно нашлось место для оптимизации.
...
Однако, после этих оптимизаций время работы не изменилось (с учётом погрешности).
|  Оптимизация | Время работы $10^6$ сравнений | Ускорение |
|---|---|---|
| `-O1` | 278.6 ± 1.9 ms | baseline |
| ASM версия | 280.8 ± 1.6 ms | 1x |

Далее было принято решение переписать данную при помощи интринсиков. Таким образом, я смог сравнивать 32 символа за раз (AVX-инструкции).
|  Оптимизация | Время работы $10^6$ сравнений | Ускорение |
|---|---|---|
| `-O1` | 278.6 ± 1.9 ms | baseline |
| ASM версия | 280.8 ± 1.6 ms | 1x |
