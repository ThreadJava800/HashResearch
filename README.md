...
|  Оптимизация | Время работы $10^5$ поисков | Относительное ускорение | Абсолютное ускорение |
|---|---|---|---|
...
|---|---|---|---|
| - | 2277.6 ± 12.1 ms | baseline | baseline | 
| Функция сравнения | 572.8 ± 2.5 ms | 4x | 4x |
...
|  Оптимизация | Время работы $10^6$ вызовов | Относительное ускорение | Абсолютное ускорение |
|---|---|---|---|
| `-O1` | 279.4 ± 0.5 ms | baseline | baseline | 
| ASM версия | 266.0 ± 0.9 ms | 1.05x | 1.05x |
...
Также стоит отметить, что ассемблерная функция, находящаяся в другом файле, с меньшей вероятностью заинлайниться, чем функция, написанная с помощью ассемблерной вставки. Это видно из тестов:
|  Оптимизация | Время работы $10^6$ вызовов | Относительное ускорение | Абсолютное ускорение |
|---|---|---|---|
| `-O1` | 279.4 ± 0.5 ms | baseline | baseline | 
| ASM версия (inline) | 266.0 ± 0.9 ms | 1.05x | 1.05x |
| ASM версия (файл) | 287.6 ± 4.8 ms | 0.92x | 0.97x |
...
| - | 2277.6 ± 12.1 ms | baseline | baseline |
| Функция сравнения | 572.8 ± 2.5 ms | 4x | 4x |
| Оптимизация хеша | 507.0 ± 9.1 ms | 1.13x | 4.5x |

### Поиск длины строки <a name="optim3"></a>
Далее я решил оптимизировать функцию поиска длины строки.\
...
В качестве оптимизации можно mov заменить на xor. Однако, функция быстрее работать быстрее не стала.\
В итоге была предпринята попытка переписать эту функцию на интринсиках. Тем не менее, в процессе было выяснено, что реализация стандартной функции strlen использует avx оптимизации. После необходимых измерений получились следующие результаты:
|  Оптимизация | Время работы $10^6$ вызовов | Ускорение |
|---|---|---|
| `-O1` | 143.0 ± 1.1 ms | baseline |
| ASM версия | 143.4 ± 1.4 ms | 1x |
| Стандартная функция | 4.4 ± 0.8 ms | 32.6x |

Таким образом, за счёт использования стандартной функции, скорость работы функции поиска изменилась следующим образом:
|  Оптимизация | Время работы $10^5$ поисков | Относительное ускорение | Абослютное ускорение |
|---|---|---|---|
| - | 2277.6 ± 12.1 ms | baseline | baseline |
| Функция сравнения | 572.8 ± 2.5 ms | 4x | 4x |
| Оптимизация хеша | 507.0 ± 9.1 ms | 1.13x | 4.5x |
| Оптимизация длины строки | 502.2 ± 3.9 ms | 1.01x | 4.54x |

Сильного ускорения и не ожидалось, т.к. эта функция вызывается всего один раз в функции поиска.

## Вывод <a name="res"></a>
Как показывает практика, использование интринсиков является наиболее эффективным методом оптимизации из всех рассмотренных в этой работе. Поэтому ассемблерные оптимизации следует рассматривать в последнюю очередь, например, если написание функции с интринсиками трудозатратно.